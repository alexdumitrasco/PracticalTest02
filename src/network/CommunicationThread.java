            ...
            try {
                ...
                if (bufferedReader != null && printWriter != null) {
                    ...
                    if (searchedWord != null && !searchedWord.isEmpty()) {
                        if (data.containsKey(searchedWord)) {
                            Log.i(Constants.TAG, "[COMMUNICATION THREAD] Getting the information from the cache...");
                            wordDef = data.get(searchedWord);
                        } else {
                            Log.i(Constants.TAG, "[COMMUNICATION THREAD] Getting the information from the webservice...");
                            HttpClient httpClient = new DefaultHttpClient();
                            Log.i(Constants.TAG, "[COMMUNICATION THREAD] word "+searchedWord);
                            Log.i(Constants.TAG, "[COMMUNICATION THREAD] word "+Constants.WEB_SERVICE_ADDRESS+searchedWord);

                            HttpPost httpPost = new HttpPost(Constants.WEB_SERVICE_ADDRESS+searchedWord);
                            //List<NameValuePair> params = new ArrayList<NameValuePair>();
                            //params.add(new BasicNameValuePair(Constants.QUERY_ATTRIBUTE, searchedWord));
                            //UrlEncodedFormEntity urlEncodedFormEntity = new UrlEncodedFormEntity(params, HTTP.UTF_8);
                            //httpPost.setEntity(urlEncodedFormEntity);
                            ResponseHandler<String> responseHandler = new BasicResponseHandler();
                            String pageSourceCode = httpClient.execute(httpPost, responseHandler);
                            if (pageSourceCode != null) {
                            	int position = pageSourceCode.indexOf("WordDefinition");
                                Log.i(Constants.TAG, "[COMMUNICATION THREAD] word info"+pageSourceCode);

                                pageSourceCode = pageSourceCode.substring(position);
                                serverThread.setData(searchedWord, pageSourceCode);
                                wordDef = pageSourceCode;
                        ...
                        if (wordDef != null) {
                            printWriter.println(wordDef);
                            printWriter.flush();
                            Log.e(Constants.TAG, "[COMMUNICATION THREAD] Info printed!");
